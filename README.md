...
![SANSPHASE · 無相](docs/assets/readme-banner.svg)
...
> 当前为 **1.0.0-rc.1 发布候选版**。计划使用的正式域名为 `www.sansphase.com`，腾讯云公网部署与实机验收尚未完成。公开仓库包含源码和测试，不包含站长账号、私有配置、真实数据库及上传文件。
...
- [x] 统一源码构建，分离真实数据与测试数据。
- [x] 本地干净副本安装、构建与 149 项测试通过。
- [x] GitHub Ubuntu 24.04 环境安装、构建与 149 项测试通过。
- [x] 提供生产启动、HTTPS 代理、健康检查和备份恢复流程。
- [ ] 完成腾讯云实机部署、域名与 HTTPS 验收。
- [ ] 在服务器验证重启恢复、异地备份、告警与容量。
- [ ] 后续完善文章可索引路由、邮件找回密码与大文件断点续传。
- [ ] 第二阶段建设社区注册与交流。

最新自动构建结果见页面顶部的工作流徽章；本地测试范围和待验收项见 [发布候选版记录](docs/RELEASE-PREPARATION-2026-09-15.md)。
