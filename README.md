...
当前版本处于 **Release Candidate（发布候选）** 阶段。源码构建与自动化测试已通过，生产环境部署和运维验收尚未完成。

| 阶段 | 状态 | 范围与验收依据 |
| --- | --- | --- |
| 工程基线 | 已完成 | 统一源码构建入口；运行数据、密钥与测试数据隔离；公开源码清单与敏感信息扫描。 |
| 自动化验证 | 已通过 | Windows 干净副本及 GitHub Ubuntu 24.04 环境完成依赖安装、构建与测试；149 项测试通过，0 失败、0 跳过。 |
| 部署准备 | 已提供 | 生产启动与构建完整性校验、Nginx HTTPS 配置、健康检查、备份恢复及回滚文档。 |
| 生产验收 | 待完成 | 腾讯云实机部署、域名与证书验证、进程重启恢复、异地备份、告警及存储容量检查。 |
