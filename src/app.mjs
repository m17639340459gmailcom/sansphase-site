...
function blogNotice() {
  ...
  const slides=notices.map((item,index)=>`<div class="blog-notice-slide" aria-hidden="${index !== blogNoticeIndex}">${item.image?`<div class="cover-frame cover-frame--notice"><img class="blog-notice-poster" src="${esc(item.image)}" alt="${esc(item.title)}" width="960" height="400"></div>`:''}<div class="blog-notice-copy"><span class="eyebrow">${icons.bell}${t("公告","NOTICE")}</span><h2>${esc(item.title)}</h2><p>${esc(item.summary)}</p>${item.link?`<a href="${esc(item.link)}" target="_blank" rel="noopener noreferrer" tabindex="${index===blogNoticeIndex?0:-1}">${t('查看公告','Read notice')} ${icons.right}</a>`:''}</div></div>`).join('');
...
window.addEventListener('author:content',async event=>{
  ...
  if(blogPhoto&&blogPhoto.getAttribute('src')!==background){const photo=new Image();photo.src=background;try{await photo.decode();blogPhoto.src=background;}catch{}}
