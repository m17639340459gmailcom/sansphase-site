...
function card(kind, item, ui) {
  ...
  return `<article class="blog-card catalog-card catalog-poster ${image ? "has-cover" : ""}"><a class="catalog-cover-link" href="${href(kind, item)}" tabindex="-1" aria-hidden="true">${image}</a><div class="catalog-card-copy"><div class="catalog-card-meta"><span class="eyebrow">${icons[catalogKinds[kind].icon]}${esc(item.category || t(...catalogKinds[kind].label))}</span>${date(item.date) ? `<time datetime="${esc(item.date)}">${date(item.date)}</time>` : ""}</div><h2><a class="catalog-card-title" href="${href(kind, item)}">${esc(title(item, t))}</a></h2><p>${esc(summary(item, t))}</p><div class="catalog-tags">${tags(item, ui)}</div><div class="catalog-card-actions"><a class="text-link catalog-read" href="${href(kind, item)}">${t("查看介绍", "Read more")}${icons.right}</a>${actions(kind, item, ui)}</div></div></article>`;
}
...
function feature(kind, item, ui, hero = false) {
  ...
  return `<article class="blog-card catalog-feature ${hero ? "catalog-hero" : ""}"><a class="catalog-cover-link" href="${href(kind, item)}" tabindex="-1" aria-hidden="true">${image}</a><div class="catalog-feature-copy"><span class="eyebrow">${esc(item.category || t(...catalogKinds[kind].label))}</span><h2><a class="catalog-feature-title" href="${href(kind, item)}">${esc(title(item, t))}</a></h2><p>${esc(summary(item, t))}</p>${hero ? `<a class="text-link" href="${href(kind, item)}">${t("查看详情", "Explore")}${icons.right}</a>` : ""}</div></article>`;
}
